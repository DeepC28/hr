...
// ปุ่มหลัก + แก้ไข/ยกเลิก (เหลือง), ลบ (แดง)
const btn = {
  primary:
  ...
  edit:
    "inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium " +
    "bg-amber-600 text-white hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500/30 " +
    "dark:bg-amber-500 dark:hover:bg-amber-400 dark:focus:ring-amber-400/30",
  danger:
    "inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium " +
...
const TabComp: React.FC = () => {
  ...
  const list = async (signal?: AbortSignal) => {
    setLoading(true);
    try {
      const r = await fetch(`/api/${ENTITY}${q ? `?q=${encodeURIComponent(q)}` : ""}`, {
        cache: "no-store",
        signal,
      });
  ...
  useEffect(() => {
    const ac = new AbortController();
    const t = setTimeout(() => list(ac.signal), 300);
    return () => {
      ac.abort();
      clearTimeout(t);
    };
  ...
  const save = async () => {
    ...
    const url = isEdit
      ? `/api/${ENTITY}?id=${encodeURIComponent(String(editing?.[pkGuess]))}`
      : `/api/${ENTITY}`;
    try {
  ...
  const askDelete = async (row: Row) => {
    ...
    const cf = await Swal.fire({
      ...
      customClass: {
        confirmButton: "inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium bg-red-600 text-white hover:bg-red-700",
        cancelButton:
          "inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium " +
          "border border-gray-300 bg-gray-50 text-gray-900 hover:bg-white " +
          "dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:hover:bg-gray-800",
      },
    });
    if (!cf.isConfirmed) return;
    try {
      const r = await fetch(`/api/${ENTITY}?id=${encodeURIComponent(String(pk))}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      });
      const j = await r.json();
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
        <div className="flex w-full sm:w-auto items-center gap-2">
          <input
            ...
            placeholder="ค้นหา..."
            className="w-full sm:w-64 rounded-xl border px-3 py-2 text-sm outline-none
                       bg-white text-gray-900 border-gray-300 placeholder:text-gray-400
                       focus:border-blue-600 focus:ring-2 focus:ring-blue-600
                       dark:bg-gray-900 dark:text-white dark:border-gray-700"
          />
              ...
              rows.map((r, idx) => (
                <tr key={idx} className="text-gray-900 dark:text-white">
                  ...
                  <td className="px-3 py-2 text-sm text-right space-x-2">
                    <button onClick={() => openEdit(r)} className={btn.edit}>แก้ไข</button>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white text-gray-900 dark:border-gray-800 dark:bg-gray-900 dark:text-white p-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setEditing(null)} className={btn.edit}>ปิด</button>
            </div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setEditing(null)} className={btn.edit}>ยกเลิก</button>
