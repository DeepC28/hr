...
function FieldInput({
  meta, value, onChange,
}: { meta: FieldMeta; value: any; onChange: (v:any)=>void }): JSX.Element {
  const common =
    "w-full rounded-xl border px-3 py-2 text-sm " +
    "bg-white text-gray-900 border-gray-300 placeholder:text-gray-400 " +
    // ตัด outline ดำ + ใช้ ring น้ำเงิน
...
const TabComp: React.FC = () => {
  ...
  const list = async (keyword: string) => {
    setLoading(true);
    try {
      const r = await fetch(`/api/${ENTITY}${keyword ? `?q=${encodeURIComponent(keyword)}` : ""}`);
  ...
  // ค้นหาอัตโนมัติ (debounce 250ms) + โหลดครั้งแรก
  useEffect(()=>{
  ...
  const validate = (): boolean => {
    ...
    if ("sub_district_id" in form && !String(form.sub_district_id).trim()) e.sub_district_id = "กรุณากรอกรหัสตำบล";
    if ("name_th" in form && !String(form.name_th).trim()) e.name_th = "กรุณากรอกชื่อตำบล (TH)";
    setErr(e);
  ...
  const askDelete = async (row: Row) => {
    ...
    try {
      // ใช้ API รวม: DELETE /api/<entity>?<pk>=<id> (หรือจะส่งใน body ก็ได้)
      const url = `/api/${ENTITY}?${encodeURIComponent(pkGuess)}=${encodeURIComponent(pk)}`;
      const r = await fetch(url, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      });
        ...
        <div className="flex w-full sm:w-auto items-center gap-2">
          <input
            ...
            className="w-full sm:w-64 rounded-xl border px-3 py-2 text-sm
                       ...
                       dark:focus:border-blue-400 dark:focus:ring-blue-400/30"
          />
          {/* ปุ่มค้นหาถูกเอาออก (พิมพ์แล้วค้นหาอัตโนมัติ) */}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-4" onClick={()=>setEditing(null)}>
          <div className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white text-gray-900 dark:border-gray-800 dark:bg-gray-900 dark:text-white p-4" onClick={(e)=>e.stopPropagation()}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={()=>setEditing(null)} className={btn.danger}>ยกเลิก</button>
